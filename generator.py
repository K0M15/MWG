...
def generate_site(output_dir='dist', minify=False, default_theme='default', default_template='page'):
    ...
    # First pass: identify blog folders and collect used themes
    blog_configs = {}
    used_themes = set()
    ...
    for root, dirs, files in os.walk(CONTENT_DIR):
        ...
        used_themes.add(config.get('theme', default_theme))
    ...
    for root, dirs, files in os.walk(CONTENT_DIR):
        ...
        for file in files:
            if file.endswith('.md'):
                ...
                # Store for index if it's a blog post
                if is_blog_section and file != 'index.md':
                    if blog_root not in all_posts:
                        all_posts[blog_root] = []
                    all_posts[blog_root].append(post_meta)
                    template_to_use = config.get('post_template', 'blog_post.html')
                else:
                    template_to_use = config.get('template', default_template)
    ...
    for rel_root, config in blog_configs.items():
        ...
        theme_name = config.get('theme', default_theme)
        template_to_use = config.get('template', 'blog_index.html')
        if not template_to_use.endswith('.html'):
            template_to_use += '.html'
        ...
        for i in range(total_pages):
            ...
            print(f"Generated Blog Index: {out_path} (Theme: {theme_name})")
...
class ChangeHandler(FileSystemEventHandler):
    ...
    def on_any_event(self, event):
        if not event.is_directory and any(event.src_path.endswith(ext) for ext in ['.md', '.html', '.css', '.md-server']):
            generate_site('dist', default_theme=self.theme)
...
def main():
    ...
    parser.add_argument('--template', default='page')
    args = parser.parse_args()

    if args.build:
        generate_site('build', minify=True, default_theme=args.theme, default_template=args.template)
        create_build_server('build')
    elif args.dev:
        generate_site('dist', default_theme=args.theme, default_template=args.template)
        port = find_available_port(8000)
        threading.Thread(target=serve_dev, args=('dist', port), daemon=True).start()
        observer = Observer()
        observer.schedule(ChangeHandler(args.theme), CONTENT_DIR, recursive=True)
        observer.schedule(ChangeHandler(args.theme), TEMPLATES_DIR, recursive=True)
        observer.schedule(ChangeHandler(args.theme), STATIC_DIR, recursive=True)
        observer.start()
        try:
